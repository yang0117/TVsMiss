# BIC high
# BIC_high: this function used to caculate the BIC value of each group of beta estimation and
...
BIC_high <- function(complete_dataset,beta_matrix){
  if ((ncol(complete_dataset)-1) != (ncol(beta_matrix))) stop("number of beta does not math complete_dataset")
  BIC_vec <- apply(cbind(0,beta_matrix), 1, BIC_high_singel_value, dataset=complete_dataset)
...
BIC_high_singel_value <- function(single_beta_est,dataset){
  ...
  bic_value <- -2*.Call("loglikelihood",single_beta_est,dataset) + sum(single_beta_est[-1] != 0)*log(nrow(dataset))*log(log(length(single_beta_est[-1])))/nrow(dataset)
  return(bic_value)
}
